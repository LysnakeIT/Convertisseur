...
var mp4_gif = require('convert').mp4_gif
const fs = require("fs");
...
router.get('/', function(req, res, next) {
    res.render('gif', { title: 'Convertisseur'});
});
...
router.post('/', asyncMiddleware(async (req, res, next) => {
    ...
    let newPath = process.cwd() + "/download/" + name[0] + ".gif"; // On récupère le chemin de notre dossier qui va jouer le role de stockage avent l'upload vers l'utilisateur (temporaire)
    const ret = await mp4_gif.convert(path, newPath) // On convertit le fichier
    res.redirect('/gif/download')
}));
