...
var json_csv = require('convert').json_csv
const fs = require("fs");
...
router.get('/', function(req, res, next) {
    res.render('csv', { title: 'Convertisseur'});
});
...
router.post('/', asyncMiddleware(async (req, res, next) => {
    ...
    let newPath = process.cwd() + "/download/" + name[0] + ".csv"; // On récupère le chemin de notre dossier qui va jouer le role de stockage avent l'upload vers l'utilisateur (temporaire)
    await json_csv.convert(path, newPath) // On convertit le fichier
    res.redirect('/csv/download')
}));
