...
router.get('/', function(req, res, next) {
    res.render('powerpoint', { title: 'Convertisseur'});
});
...
router.post('/', asyncMiddleware(async (req, res, next) => {
    ...
    let newPath = process.cwd() + "/download/" + name[0] + ".pptx"; // On récupère le chemin de notre dossier qui va jouer le role de stockage avent l'upload vers l'utilisateur (temporaire)
